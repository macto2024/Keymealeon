...
```
TypeError: cart total is not a number — API returned price as "$149.00"
```
...
| **0:20** | 25s | Type an edit in `src/format.js`, don't save | ~~Run Tests~~ · Git Diff · Problems · Ask Codex · More · Context | **U dims:** *"Save your files before running tests."* |
...
| **2:25** | 30s | Press **O** → the reported type error | **Console · Revert Codex · View Diff · Ask Codex + error · More · Back** | ← the payoff |
| **2:55** | 20s | Press **J**. Codex gets the real error, fixes it. `Total: $188.98` | Git Diff · Run Tests · Output · Codex · More · Context | Verified in both places |
...
**2:55** — "Now it has what it was missing. One eighty-eight ninety-eight."
...
**Real, now built** — the browser error. `demo/` is a working checkout page whose test suite goes green on the agent's fix while the page stays broken, and `demo/src/report.js` reports that to the backend the way a Sentry SDK would. The wall is a property of the code, not of the script.

**Staged** — `Revert Codex` and the commit layer at 3:15.

## Production notes

- **Cut 3:15 first if over time.** The argument is complete when the browser shows `$188.98`. Ending on "and then we committed" is weaker than ending on the claim.
