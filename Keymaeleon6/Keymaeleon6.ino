...
// Physical layout: 1 2 3 / 4 5 6. Standalone animation/button test, no HID.
...
void command(const char *text) {
  ...
  if (!strcmp(text, "HELLO")) logLine("KEYMAELEON6 DUAL_I2C DEBUG_2 WAVE 8 120\n");
...
void setup() {
  ...
  for (uint8_t row = 0; row < 2; ++row) {
    ...
    if (!debugQueue[row] || !rows[row].queue || xTaskCreatePinnedToCore(displayWorker,
