...
This revision returns to **two buses/two muxes**, with the software isolation fix applied. It does not establish that all reported hardware symptoms are fixed. Wiring is in [README.md](README.md).
...
After uploading with **Wave.h and IsolatedDisplay.h** present, connect all six OLEDs as documented. Serial Monitor: **115200, newline**. Send commands individually:
...
Check `HELLO` contains `DUAL_I2C DEBUG_2`. Diagnostic commands finish with **two DONE lines**, tagged `BUS 0` and `BUS 1`; wait for both. HELLO/STATS do not produce DONE. Workers can print their lines in either order.
