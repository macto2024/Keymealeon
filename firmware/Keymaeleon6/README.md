...
This revision fixes the earlier implementation's U8g2 shared-memory/shared-transfer-state hazards: `IsolatedDisplay.h` binds an independently owned framebuffer and selects the stateless classic SSD13xx command callback. The user confirmed on 2026-09-19 that replacing faulty displays resolved the mux2 hardware fault. The diagnostics remain available for future checks.

Each press runs the Animation Lab bottom wave: **8 pixels thick, 120 pixels/second**, over the current action pictogram. Waves stack independently, merge on overlap and finish after 450 ms. The sketch sends revision-tagged key presses to `companion/keymaeleon_6.py`, which executes Ubuntu X11 actions. It does not send USB HID keystrokes. Without the companion, displays show Idle; local waves and diagnostics still work. See the [six-key launch instructions and layouts](../../README.md#six-key-version--quick-start).
...
3. Open `Keymaeleon6.ino`, keeping **Wave.h, IsolatedDisplay.h and Icons6.h** alongside it.
...
`HELLO` must report `KEYMAELEON6 DUAL_I2C COMPANION_1 WAVE 8 120`.
...
## Companion protocol and artwork

`HELLO` identifies capability `COMPANION_1`. The host sends `RUN`, then `SET6 <revision> <profile>` every 500 ms; profile IDs are Idle=0, Firefox=1, VS Code=2, Terminal=3, Spotify=4, Desktop=5. Revisions are nonzero 32-bit values. Each row receives the latest layout through its own mailbox; the existing bus/worker/framebuffer isolation remains intact.

A button-down produces `PRESS6 <sequence> <displayed-revision> <key>` with key 0–5, alongside the diagnostic BUTTON line. The displayed revision is recorded after a successful frame transfer. The companion ignores repeated sequences, stale revisions, unavailable focus and unknown apps. Firmware sends `BOOT6` on startup, and returns to Idle after 1.5 seconds without SET6. Diagnostics that pause rendering also suppress companion press events; stop the companion before using Serial Monitor.

`tools/generate_six_icons.py` generates `assets/icons_6.json` and `Icons6.h` from the original icons plus six-key artwork. Run it with Pillow installed after changing layouts/artwork, then reflash. Runtime Python needs no Pillow. The simulator and firmware use the same generated pixels and profile order.

### Live VS Code pictographs

The bridge adds capability `ICON6_1` and `ICON6 <revision> <id0> … <id5>`.
Each ID indexes the generated `sixIconCatalog` in `Icons6.h`; the companion uses
identically ordered `assets/icons_6.json` icons. All six IDs are validated before
one layout is sent to both workers. Profile 8 is reserved for this atomic dynamic
layout; original SET6 profiles, generated text, waves, stale-press rejection and
watchdog behavior remain intact. Regenerate both assets with
`python3 tools/generate_six_icons.py` and reflash after changing the catalogue.

`ICON6_LABELS_1` adds VS Code-only icon-and-label bitmaps to the catalogue. The
icon occupies the top 22 pixels; a centered 5×7 uppercase label occupies rows
25–31. The complete bitmap, including its label, participates in the existing
wave animation. Other app profiles retain their original full-size pictograms.
Flash the updated sketch with `Icons6.h` to enable labels; the companion keeps
using the original icon IDs on older `ICON6_1` firmware and logs an upgrade hint.
